...
/**
 * Имплементация на стратегия за извличане на обяви за работа от "<a href="https://www.itjobsworldwide.com/jobs/backend-development">...</a>".
 * Класът реализира интерфейса {@link CrawlerStrategy} и предоставя логиката за извличане на обяви от даден HTML документ.
 */
public class ItJobsWorldwideCrawler implements CrawlerStrategy {
    ...
    public List<JobListing> extractJobListings(Document doc, String website) {
        ...
        doc.select(".results-list").forEach(element -> {
            System.out.println("\n---------- Fetching jobs from: " + website);
            List<String> jobTitles = element.select(".job-title").eachText();
            List<String> jobUrls = element.select(".job-title a").eachAttr("href");
            ...
            for (int i = 0; i < jobTitles.size(); i++) {
                ...
                if (!jobUrl.startsWith("http")) {
                    jobUrl = "https://www.itjobsworldwide.com" + jobUrl;
                }
